...
static int cfg_parse_peers_bind(char **args, int section_type, struct proxy *curpx,
...
static int cfg_parse_peers_default_server(char **args, int section_type, struct proxy *curpx,
                                          const struct proxy *defpx, const char *file, int linenum,
                                          char **err)
{
	int err_code = 0;
...
/* Parse "log" keyword */
static int cfg_parse_peers_log(char **args, int section_type, struct proxy *curpx,
                               const struct proxy *defpx, const char *file, int linenum,
                               char **err)
{
	int err_code = 0;
	...
	if (!parse_logger(args, &curpeers->peers_fe->loggers, (strcmp(args[0], "no") == 0), file, linenum, &errmsg)) {
...
static int cfg_parse_peers_section(char **args, int section_type, struct proxy *curpx,
                                   const struct proxy *defpx, const char *file, int linenum,
                                   char **err)
{
	const char *errc;
...
static int cfg_parse_peers_peer(char **args, int section_type, struct proxy *curpx,
                                const struct proxy *defpx, const char *file, int linenum,
                                char **err)
{
	struct peer *newpeer = NULL;
...
static int cfg_parse_peers_shards(char **args, int section_type, struct proxy *curpx,
                                  const struct proxy *defpx, const char *file, int linenum,
                                  char **err)
{
	char *endptr;
...
/* Parse "table" keyword */
static int cfg_parse_peers_table(char **args, int section_type, struct proxy *curpx,
                                 const struct proxy *defpx, const char *file, int linenum,
                                 char **err)
{
...
static int cfg_parse_peers_disabled(char **args, int section_type, struct proxy *curpx,
                                    const struct proxy *defpx, const char *file, int linenum,
                                    char **err)
{
...
static int cfg_parse_peers_enabled(char **args, int section_type, struct proxy *curpx,
                                   const struct proxy *defpx, const char *file, int linenum,
                                   char **err)
{
	curpeers->disabled = 0;
	...
	list_for_each_entry(kwl, &cfg_keywords.list, list) {
		for (index = 0; kwl->kw[index].kw != NULL; index++) {
			if ((kwl->kw[index].section == CFG_PEERS) &&
				...
				rc = kwl->kw[index].parse(args, CFG_PEERS, curpeers ? curpeers->peers_fe : NULL, NULL, file, linenum, &errmsg);
				if (rc < 0) {
					ha_alert("parsing [%s:%d] : %s\n", file, linenum, errmsg);
					err_code |= ERR_ALERT | ERR_FATAL;
					goto out;
