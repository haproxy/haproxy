...
int cpu_detect_topology(void)
{
	...
	skip_cache:
		 ...
		 */
		if (no_clust < 0)
			no_clust = !is_dir_present(NUMA_DETECT_SYSTEM_SYSFS_PATH "/cpu/cpu%d/topology/cluster_cpus_list", cpu);
		 ...
		 */
		if (no_die < 0)
			no_die = !is_dir_present(NUMA_DETECT_SYSTEM_SYSFS_PATH "/cpu/cpu%d/topology/die_cpus_list", cpu);
	...
	skip_topo:
		if (no_capa < 0)
			no_capa = !is_dir_present(NUMA_DETECT_SYSTEM_SYSFS_PATH "/cpu/cpu%d/cpu_capacity", cpu);
