...
static int sample_conv_ip_data(const struct arg *arg_p, struct sample *smp, void *private)
{
	...
	/* len points to the end of the header */
 ...
 */
static int sample_conv_tcp_options(const struct arg *arg_p, struct sample *smp, void *private)
{
	...
	while (ofs < len) {
		...
		/* kind1 = NOP and is a single byte, others have a length field */
		ofs += (smp->data.u.str.area[ofs] == 1) ? 1 : smp->data.u.str.area[ofs + 1];
	}
...
static struct sample_conv_kw_list sample_conv_kws = {ILH, {
	...
	{ "tcp.dst",            sample_conv_tcp_dst,            0,      NULL,      SMP_T_BIN,  SMP_T_ADDR },
	{ "tcp.flags",          sample_conv_tcp_flags,          0,      NULL,      SMP_T_BIN,  SMP_T_SINT },
	{ "tcp.options",        sample_conv_tcp_options,        0,      NULL,      SMP_T_BIN,  SMP_T_BIN  },
	...
	{ "tcp.src",            sample_conv_tcp_src,            0,      NULL,      SMP_T_BIN,  SMP_T_ADDR },
