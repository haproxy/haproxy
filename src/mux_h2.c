...
/* Returns non-zero if the h2s is guaranteed to be allowed to allocate an rxbuf
 * slot from its connection, otherwise zero. This is in order to ease early
 * checks before engaging into any actual work.
 */
...
static inline uint h2s_get_rxbuf(struct h2s *h2s)
{
	...
	if (!h2s_may_get_rxbuf(h2s)) {
		/* FIXME: for now this must never happen since there's always
		 * enough slots for any connection.
		 */
		BUG_ON_HOT(1, "h2s_may_get_rxbuf() returned false");
		return 0;
	}
...
static int h2_frt_transfer_data(struct h2s *h2s)
{
 ...
 next_buffer:
	h2c->flags &= ~H2_CF_DEM_RXBUF;
	if ((!h2s_rxbuf_tail(h2s) || (full || !h2s_may_append_to_rxbuf(h2s))) &&
	    h2s_may_get_rxbuf(h2s) && !h2s_get_rxbuf(h2s)) {
		h2c->flags |= H2_CF_DEM_RXBUF;
...
static int h2_dump_h2s_info(struct buffer *msg, const struct h2s *h2s, const char *pfx)
{
	...
	chunk_appendf(msg, " h2s.id=%d .st=%s .flg=0x%04x .rxbuf.c=%u .t=%u@%p+%u/%u .h=%u@%p+%u/%u",
...
static int init_h2()
{
	...
	max_streams = h2_fe_settings_max_concurrent_streams ?
	              h2_fe_settings_max_concurrent_streams :
	              h2_settings_max_concurrent_streams;
	...
	pool_head_h2_rx_bufs = create_pool("h2_rx_bufs",
	                                   (max_streams + 1) * sizeof(struct bl_elem),
	                                   MEM_F_SHARED|MEM_F_EXACT);
