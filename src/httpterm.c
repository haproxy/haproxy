...
static int hstream_htx_buf_rcv(struct connection *conn, struct hstream *hs)
{
 ...
 leave:
	...
	TRACE_PRINTF(TRACE_LEVEL_PROTO, HS_EV_HSTRM_RECV, hs, 0, 0, 0,
	             "data received (%llu) ret=%d read=%d", (unsigned long long)cur_read, ret, (int)read);
 ...
 */
static inline int hstream_may_rcv(struct hstream *hs)
{
	return hs->req_body > 0 && (!hs->req_after_res || !hs->to_write);
}
...
static struct task *process_hstream(struct task *t, void *context, unsigned int state)
{
 ...
 receive:
		...
		if (!hs->to_write && hs->req_after_res && hs->req_body &&
