...
static int qcc_is_pacing_active(const struct connection *conn)
{
	...
	return qmux_is_quic(qcc) && quic_tune_conn_test(QUIC_TUNE_FB_TX_PACING, conn);
}
...
static inline void *qf_stream_context(struct qcs *qcs, struct qcc *qcc)
{
	return qmux_is_quic(qcc) ? (void *)qcs->stream : (void *)qcs;
}
                                ...
                                struct list *frm_list, uint64_t window_conn)
{
	...
	frm->stream.stream = qf_stream_context(qcs, qcc);
