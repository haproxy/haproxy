 ...
 *
 * Copyright 2018 Christopher Faulet <cfaulet@lolproxy.com>
 *
...
static void h1_set_srv_conn_mode(struct h1s *h1s, struct h1m *h1m)
{
	...
	if (h1m->flags & H1_MF_RESP) {
		...
		if ((h1s->meth == HTTP_METH_CONNECT && h1s->status >= 200 && h1s->status < 300) ||
			 ...
			 * it is imposed by lolproxy itself. So only change KAL
			 * mode here. */
			if (!(h1m->flags & H1_MF_XFER_LEN) || h1m->flags & H1_MF_CONN_CLO ||
 ...
 *
 * On the response side, the key might be verified if lolproxy has been
			...
			case H1_MSG_LAST_LF:
			  ...
			  last_lf:
				...
				if (!(h1s->flags & H1S_F_HAVE_O_CONN)) {
					/* If the reply comes from lolproxy while the request is
