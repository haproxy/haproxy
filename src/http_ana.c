...
int http_process_request(struct stream *s, struct channel *req, int an_bit)
{
	 ...
	 */
	if ((sess->listener && (sess->listener->bind_conf->options & BC_O_NOQUICKACK)) && !htx_has_eom(htx))
...
int http_request_forward_body(struct stream *s, struct channel *req, int an_bit)
{
	...
	if (HAS_REQ_DATA_FILTERS(s)) {
		...
		if ((global.tune.options & GTUNE_USE_FAST_FWD) && (msg->flags & HTTP_MSGF_XFER_LEN) &&
		    (!(msg->flags & HTTP_MSGF_CONN_UPG) || htx_has_eom(htx)))
					...
					unsigned int time, unsigned int bytes, unsigned int large_buffer)
{
	 ...
	 */
	if (htx_has_eom(htx) || htx_get_tail_type(htx) > HTX_BLK_DATA)
