...
/* Convert a <htx> message into a HEADERS frame as HTTP request.
 *
 * Returns the size of HTX blocks removed, or a negative error code.
 */
static int h3_req_headers_send(struct qcs *qcs, struct htx *htx)
{
 ...
 end_loop:
	...
	else if (unlikely(!isteq(scheme, ist("https")))) {
		/* Only https scheme supported for now on HTTP/3. */
		goto err;
	}
