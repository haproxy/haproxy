 ...
 * this function is not protected by a lock, so the caller is responsible to
 * properly use idle_conns_lock prior to calling it. A notable difference is
 * also that <owner> member of <conn> is not reset.
 *
 * This function should be reserved for idle connections temporarily removal
 * for protection against other thread access. It is generally coupled with a
 * call to session_reinsert_idle_conn() performed before task scheduling.
 */
