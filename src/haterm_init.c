...
static void haterm_usage(char *name)
{
	fprintf(stderr,
		...
		"        -T <line> : multiple option; need -d; append <line> to the \"traces\" section\n"
...
void haproxy_init_args(int argc, char **argv)
{
	...
	if (!hbuf_is_null(&tbuf) && !haterm_debug) {
		ha_warning("ignored -T option, depends on missing -d option\n");
		free_hbuf(&tbuf);
	}
