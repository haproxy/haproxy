			...
			if (ret > 0 && msg_controllen > 0) {
				...
				if (cmsg->cmsg_type == TLS_GET_RECORD_TYPE) {
					...
					if (record_type == SSL3_RT_HANDSHAKE && tmpbuf[0] == SSL3_MT_NEW_SESSION_TICKET) {
						 ...
						 */
						if (record_type == SSL3_RT_ALERT && ret == 2 && tmpbuf[0] == SSL_TLSEXT_ERR_ALERT_WARNING && tmpbuf[1] == SSL_AD_CLOSE_NOTIFY) {
							b_del(buf, ret);
							goto read0;
						} else {
...
static int ssl_sock_get_capability(struct connection *conn, void *xprt_ctx, enum xprt_capabilities cap, void *arg)
{
	...
	switch (cap) {
		case XPRT_CAN_SPLICE:
			ret = arg;
			if ((ctx->flags & (SSL_SOCK_F_KTLS_RECV | SSL_SOCK_F_KTLS_SEND)) == (SSL_SOCK_F_KTLS_RECV | SSL_SOCK_F_KTLS_SEND)) {
