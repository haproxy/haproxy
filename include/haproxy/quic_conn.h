...
{
	struct server *s = qc->conn ? objt_server(qc->conn->target) : NULL;

	return qc_is_listener(qc) ?
		qc->li->bind_conf->quic_params.max_udp_payload_size :
		s ? s->quic_params.max_udp_payload_size : 0;
}
