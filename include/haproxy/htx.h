...
static inline void htx_dump(struct buffer *chunk, const struct htx *htx, int full)
{
	...
	for (pos = htx_get_head(htx); pos != -1; pos = htx_get_next(htx, pos)) {
		...
		if (type == HTX_BLK_REQ_SL || type == HTX_BLK_RES_SL) {
			sl = htx_get_blk_ptr(htx, blk);
			chunk_appendf(chunk, "\t\t[%u] type=%-17s - flags=0x%02x - size=%-6u - addr=%-6u\t%.*s %.*s %.*s\n",
			...
			chunk_appendf(chunk, "\t\t[%u] type=%-17s - flags=0x%02x - size=%-6u - addr=%-6u\t%.*s: %.*s\n",
				      pos, htx_blk_type_str(type), blk->flags, sz, blk->addr,
				      (int)MIN(n.len, 32), n.ptr,
				      (int)MIN(v.len, 64), v.ptr);
		else
			chunk_appendf(chunk, "\t\t[%u] type=%-17s - flags=0x%02x - size=%-6u - addr=%-6u%s\n",
				      pos, htx_blk_type_str(type), blk->flags, sz, blk->addr,
				      (!v.len ? "\t<empty>" : ""));
	}
}
