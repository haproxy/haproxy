 ...
 *       expiration date.
 *
 * <kip_len> is the known input length still expected from the producer. It is
 *           set by the stream endpoint and decremented once consumed by the app
 *           loyer. Depending on the enpoint, this value may be unset. It may be
 *           set only once if the payload lenght is fully known from the
 *           begining (a HTTP message with a content-length for instance), or
 *           incremented periodically when more data are expected (a
 *           chunk-encoded HTTP message for instance). On the app side, this
 *           value is decremented when data are scheduled to be forwarded to the
 *           other side and <kop_len> is incremented accordingly.
 *
 * <kop_len> is the known output length still expected from the app layer. It is
 *           set by the app layer, when data are forwarded to the stream
 *           endpoint and decremented when the endpoint the value was processed
 *           (for instance when a size of the next chunk is emitted).
 *
 * NOTE: <kip_len> and <kop_len> act like communicating vessels and are just
 *       used as an information from the producer side to allow optimisations on
 *       the consumer side. These value may be unset by the producer or just
 *       never used by the consumer. When set, these values reflect the data
 *       already present in the channel buffer, and those which will come soon.
 *
 */
struct sedesc {
	...
	unsigned long long kip_len; /* Known input payload length still expected (see above) */
	unsigned long long kop_len; /* Known outgoing payload length still expected (see above) */
};
